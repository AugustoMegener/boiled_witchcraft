...
class GlyphStack(glyph: Glyph = placeholder, innerStack: GlyphStack? = null) : INBTSerializable<CompoundTag> {
    ...
    fun canSetInnerStack(stack: GlyphStack) = stack.data.size < data.size
    ...
    fun canPut(pos: Vector2i, stack: GlyphStack) =
        stack.data.size < data.size && !glyphStacks.containsKey(pos) && glyph.getSignal(data)[pos] ?: false
