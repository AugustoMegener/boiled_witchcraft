...
class GlyphOnAPaper(properties: Properties) : Item(properties) {
    ...
    private fun getGlyphType(stack: ItemStack) = fromID(
        (stack.tag.also { if (it==null) setGlyphType(stack, GlyphType.placeholder) })!!.getString("glyph_type")
    )!!

    private fun getGlyphData(stack: ItemStack) = getGlyphType(stack).newData().also { data ->
        data.deserializeNBT((stack.tag.also { if (it==null) setGlyphData(stack, data) })!!.getCompound("glyph_type"))
    }
