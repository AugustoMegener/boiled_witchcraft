...
object GlyphCanvasManager :
    SimpleJsonResourceReloadListener(GsonBuilder().setPrettyPrinting().create(), "canvas")
{
    ...
    private fun debugSkipping(resourceLocation: ResourceLocation, exc: RuntimeException) {
        logger.debug("Skipping loading canvas $resourceLocation as it's conditions were not met", exc)
    }
}