...
abstract class CanvasRegistry<T, R>(dir: String) : ResourceRegistry<T, R>(dir) {
    final override fun makeRegistry(location: ResourceLocation, json: JsonElement) =
        makeCanvasRegistry(location,
                json.asJsonObject["markers"].asJsonArray.map { GlyphCanvasMarker.valueOf(it.asString.uppercase()) }, json.asJsonObject[dir])
